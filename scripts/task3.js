...
let subtractButton = document.getElementById('subtractNumbers');
...
let odds = numArray.filter(isOdd);
...
let evens = numArray.filter(isEven);
...
let arraySum = numArray.reduce(sum, 0);

document.getElementById('sumOfArray').textContent = arraySum;
...
let arrayProduct = numArray.map(multiplyArray);

document.getElementById('multiplied').textContent = arrayProduct;

// Step 7: Use the map and reduce array methods to sum the array elements after multiplying each element by two.  Assign the result to the HTML element with an ID of "sumOfMultiplied"

let arrayProductSum = numArray.map(multiplyArray);
arrayProductSum = arrayProductSum.reduce(sum, 0);

document.getElementById('sumOfMultiplied').textContent = arrayProductSum;